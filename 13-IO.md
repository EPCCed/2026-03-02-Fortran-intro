...
Writing data to, or reading data from, an external file is an essential
part of a useful application.

## Writing a file

File handles, or *unit numbers* are obtained using `open()`, and are
...
A valid unit number can be assigned by using the `newunit` option to `open()` --
this has been available since Fortran 2008. There is no need to choose your own
(and it can be error-prone to do so).

Note that a given file can only be *connected* to one unit number at
any given time.
...
expressed as an ASCII file. See the description at
[Wikipedia](https://en.wikipedia.org/wiki/Netpbm).

In the template [program1.f90](files/exercises/13-IO/program1.f90), we establish a
two-dimensional logical array of a fixed size, and we want to write out a file
that can be viewed in "P1" format (file extension `.pbm` ) by an image viewer.
...
You can check any image you produce by either downloading it to your machine
with `scp` or `rsync`, or you can view it directly on ARCHER2 by logging in
with X11 forwarding enabled, running `module load imagemagick` and then
`display test49.pbm`.

:::::::::::::::  solution

A [solution](files/exercises/18-further-functions/pbm_image.f90) to this problem
appears in the later [episode on interfaces](18-further-functions.md).

You should be able to produce an output PBM that looks like the example
shown below.

![](fig/test49-pbm.png){alt='Sample PBM output.'}
